...
*(48 / 2(9+3) 입력 시 곱셈 기호 자동 삽입 및 결과 288 출력 화면)*

*(루트, 제곱, 퍼센트 등 단항 연산 수행 화면)*

*(복합 다중 연산 및 괄호 우선순위 적용 화면)*

 - 과제 내용

     - **다중 연산 및 괄호 지원**: `3 + 4 * 2`와 같이 여러 연산자가 포함된 수식을 한 번에 계산하고, `( )` 괄호를 통해 연산 우선순위를 제어하는 기능을 추가합니다.
     - **공학용 기초 함수 구현**: 숫자 하나에 즉각 적용되는 **루트($\sqrt{x}$)**, **제곱($x^2$)**, **분수($1/x$)**, **퍼센트($\%$)** 버튼을 구현합니다.
     - **입력 편의성 개선**: 숫자와 괄호 사이의 곱셈 기호 생략(`2(3+1)` → `2*(3+1)`) 처리 및 숫자 연속 입력 시 결과창 실시간 갱신 로직을 고도화합니다.

 - 구현 내용과 기능 설명
 
     - **수식 계산 엔진 (DataTable.Compute)**: 기존의 이항 연산(`firstNumber`, `secondNumber`) 방식에서 탈피하여, `System.Data.DataTable`의 `Compute` 메서드를 활용한 **수식 파싱(Parsing) 방식**으로 전환했습니다. 이를 통해 사칙연산의 수학적 우선순위와 괄호 처리를 완벽하게 지원합니다.
     - **곱셈 기호 자동 삽입 (Regex)**: `System.Text.RegularExpressions`를 활용하여 사용자가 `2(9+3)`과 같이 연산자를 생략하고 입력하더라도, 내부적으로 숫자와 괄호 사이에 `*` 기호를 자동 삽입하여 계산 오류를 방지했습니다.
     - **실시간 숫자 뭉치 추출**: `NumberButton_Click` 메서드에서 `Split` 로직을 개선하여, 수식 전체가 아닌 **현재 입력 중인 마지막 숫자 뭉치**만 실시간으로 `txtResult`에 표시하도록 수정했습니다. (예: `48 + 12` 입력 시 결과창에 `12` 표시)
     - **단항 연산 처리**: `btnRoot_Click`, `btnSquare_Click` 등은 현재 `txtResult`에 표시된 값을 `double`형으로 변환 후 `Math` 클래스를 사용하여 즉시 계산하고, 수식 창에 그 과정을 기록하도록 구현했습니다
